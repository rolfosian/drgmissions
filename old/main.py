...
with open('netlifytoken.txt', 'r') as f:
    netlifytoken = f.read().strip()
    
with open('siteID.txt', 'r') as f:
    siteID = f.read().strip()

headers = {
    "Content-Type" : "application/zip", 
    "Authorization": f"Bearer {netlifytoken.strip()}"
    }
...
def render_beer(beer):
    #beers = {
        #'Backbreaker Stout': './img/Icons_BackBreaker_Label.png',
        #'Dark Morkite': './img/Dark_morkite_label.png',
        #"Pots O' Gold": './img/Icons_PotsoGold_Label.png',
        #'Red Rock Blaster': './img/Red_rock_blaster_label.png',
        #'Rocky Mountain': './img/Icons_RockyMountain_Label.png',
        #'Skull Crusher Ale': './img/Skull_crusher_ale_label.png',
        #'Slayer Stout': './img/Icons_Slayer_Stout_Label.png',
        #'Tunnel Rat': './img/Icons_TunnelRat_Label.png'
        #}
    #beer = beers[beer]
    #beer = Image.open(beer)
    #beer = scale_image(beer, 0.16)
    #return beer
...
def render_index(DRG):
    ...
    img_count = 0
    Biomes = DRG['Biomes']
    #dailyspecial = DRG['Daily Special']
    #dailyspecial.save('./index/dailyspecial.png')
                    ...
                    document.addEventListener("DOMContentLoaded", function() {
                        ...
                        const countdownTimer = setInterval(() => {
                            ...
                            if (remainingTime < 0) {
                                ...
                                document.getElementById("ddcountdown").innerHTML = "EXPIRED";
                            } else {
                        ...
                        document.addEventListener("DOMContentLoaded", function() {
                            ...
                            const countdownTimer1 = setInterval(updateCountdown1, 1000);
                                function updateCountdown1() {
                                    const remainingTime1 = Math.floor(((targetTime1 - new Date() + 4) / 1000));
                                    if (remainingTime1 <= 0) {
                                        clearInterval(countdownTime1);
                                    countdownElement1.textContent = "REFRESH THE PAGE FOR NEW MISSIONS";
                                    return;
                                }
    ...
    html += '           </h2>\n'
    html += '           <hr>\n'
    html += '         <a class="jsonlink" href="/drgmissions.json"><strong>DRG DATA</strong></a>\n'
...
with open('drgmissions.json', 'r') as f:
    DRG = f.read()
    
DRG = json.loads(DRG)

#beer = DRG['Daily Special']
#beer = render_beer(beer)
#DRG['Daily Special'] = beer
...
html = render_index(DRG)

with open('index.html', 'w') as f:
    f.write(html)

shutil.copy('drgmissions.json', './index/drgmissions.json')
shutil.copy('BebasNeue-Regular.woff2', './index/BebasNeue-Regular.woff2')
shutil.copy('HammerBro101MovieThin-Regular.woff', './index/HammerBro101MovieThin-Regular.woff')
shutil.copy('index.html', './index/index.html')
command = ["7z", "a", 'index.zip', 'index']
subprocess.run(command)
with open('index.zip', 'rb') as f:
    data = f.read()
os.remove('index.zip')
response = requests.post(f'https://api.netlify.com/api/v1/sites/{siteID.strip()}/deploys', headers=headers, data=data)
