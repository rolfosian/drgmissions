...
var biomeBanners = {
    ...
    "Ossuary Depths" : `${domainURL}/static/img/DeepDive_MissionBar_OssuaryDepths.webp`
};
...
function sliceCorners(img, sliceSize = 50) {
    return new Promise(resolve => {
        ...
        ctx.save()
        ctx.beginPath()
        ctx.moveTo(sliceSize, 0)
        ctx.lineTo(img.width - sliceSize, 0)
        ctx.lineTo(img.width, sliceSize)
        ctx.lineTo(img.width, img.height - sliceSize)
        ctx.lineTo(img.width - sliceSize, img.height)
        ctx.lineTo(sliceSize, img.height)
        ctx.lineTo(0, img.height - sliceSize)
        ctx.lineTo(0, sliceSize)
        ctx.closePath()
        ctx.clip()
...
function arrayBiomes(Biomes, season) {
    ...
    let biomes_ = ['Crystalline Caverns', 'Glacial Strata', 'Radioactive Exclusion Zone', 'Fungus Bogs', 'Dense Biozone', 'Salt Pits', 'Sandblasted Corridors', 'Magma Core', 'Azure Weald', 'Hollow Bough', "Ossuary Depths"];
...
function drawText(ctx, text, x, y, fillStyle, fontSize, fontName) {
...
function addShadowedTextToCenterOfImage(canvas, text, fontSize, fontName) {
    ...
    const textWidth = getTextMetrics(tempCtx, text, `${fontSize}px ${fontName}`).width
    const targetX = x - textWidth / 2

    shadowText(tempCtx, text, targetX, y+15, "black", fontSize, fontName)
    drawText(tempCtx, text, targetX, y+15, "white", fontSize, fontName)
...
function addShadowedTextForDeepDiveCodeName(canvas, texts, fontSize) {
    ...
    const descriptorFontName = `RiftSoft-Regular`;
    const mainFontName = `BebasNeue`;
    ...
    shadowText(tempCtx, descriptorText, combinedX, y+15, 'black', fontSize, descriptorFontName);
    drawText(tempCtx, descriptorText, combinedX, y+15, 'white', fontSize, descriptorFontName);
...
function renderDeepDiveBiomeCodename(biome, codename) {
    ...
    for (let biomeName in biomeBanners) {
        if (biomeName == biome) {
            ...
            canvas.height = img.height;
            ctx.drawImage(img, 0, 0);
            addShadowedTextForDeepDiveCodeName(canvas, texts, fontSize);
            break
        }
    }
    return canvas;
...
async function initialize(date) {
    ...
    <h2>
    <div class="biome-container" biome="Ossuary Depths">
    <br><div id="Ossuary Depths" class="missions">
    </div>
    </div>
    </h2>
    ...
    <h2>
    <div class="biome-container" biome="Ossuary Depths">
    <br><div id="nextOssuary Depths" class="missions">
    </div>
    </div>
    </h2>
